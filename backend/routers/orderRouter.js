...
orderRouter.get('/:id', isAuth, expressAsyncHandler(async(req, res) =>{
    ...
    if(order){
        ...
        req.status(404).send({ message: 'đơn hàng không tồn tại'});
    }
}));
...
orderRouter.put(':/id/pay', isAuth, expressAsyncHandler(async(req, res) =>{
    ...
    if(order){
        ...
        res.send({message: 'Thanh toán thành công', order: updatedOrder});
    } else{
        res.status(404).send({message: 'Thanh toán thất bại'});
    }
}));
